//CMPE360 PROJECT7//
//BERKE BEYAZBENLİ(10022751132) AND EREN KAAN ÇAKIR(14179079828)
//----------------------------------------------------------------------------
...
function render() {
   ...
   p = ortho(-4.0, 4.0, -4.0, 4.0, -10.0, 10.0);
   //p = ortho(left, right, bottom, top, near, far);
   gl.uniformMatrix4fv(projLoc, gl.FALSE, flatten(transpose(p)));
   // Draw the axes
  // var mvAxes = lookAt(eye, at, up);
   mv = lookAt(eye, at, up);
   mv = rotate(45, [1, 1, 0]); // Rotate 45 degrees around the axis (1, 1, 0)
   gl.uniformMatrix4fv(mvLoc, gl.TRUE, flatten(transpose(mv)));
   //var mvAxes = mult(mv, rotate(0, [1, 0, 0]));
   //gl.uniformMatrix4fv(mvLoc, gl.TRUE, flatten(transpose(mvAxes)));
   gl.drawArrays(shapes.axes.type, shapes.axes.start, shapes.axes.size);

   // Draw the first wireCube centered at (1, 0, 0)
   var mv1 = mult(mv, translate(1.0, 0.0, 0.0));
   ...
   // Draw the second solidCube (colored blue) rotated 45 degrees around the y-axis
 //Comment out the rendering of the first cube
 

// Render only the second cube with basic translation
   // Draw the second solidCube (colored blue) rotated 45 degrees around the y-axis
   var mv2 = mult(mv, translate(1.0, 1.0, 0.0));
   mv2 = mult(mv2, rotate(45, [0, 1, 0]));  // Rotate 45 degrees around the y-axis
